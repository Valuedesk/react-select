...
var React = require("react"), React__default = _interopDefault(React), core = require("@emotion/core"), reactDom = require("react-dom"), PropTypes = _interopDefault(require("prop-types")), utils = require("./utils-a1338763.cjs.prod.js"), _css = _interopDefault(require("@emotion/css")), AutosizeInput = _interopDefault(require("react-input-autosize"));
...
function getMenuPlacement(_ref) {
  ...
  var _scrollParent$getBoun = scrollParent.getBoundingClientRect(), scrollParentTop = _scrollParent$getBoun.top, scrollParentBottom = _scrollParent$getBoun.bottom, _menuEl$getBoundingCl = menuEl.getBoundingClientRect(), menuBottom = _menuEl$getBoundingCl.bottom, menuHeightBoundingRect = _menuEl$getBoundingCl.height, menuTop = _menuEl$getBoundingCl.top, menuHeight = menuEl.style.transform ? menuEl.offsetHeight : menuHeightBoundingRect, viewHeight = scrollParent.clientHeight, scrollTop = utils.getScrollTop(scrollParent), marginBottom = parseInt(getComputedStyle(menuEl).marginBottom, 10), marginTop = parseInt(getComputedStyle(menuEl).marginTop, 10), viewSpaceAbove = menuTop - scrollParentTop - scrollTop, viewSpaceBelow = viewHeight - menuTop, scrollSpaceAbove = viewSpaceAbove + scrollTop, scrollSpaceBelow = scrollParent.scrollHeight - scrollTop - viewHeight, scrollSpaceBelowMenu = scrollParentBottom - menuTop, scrollDown = menuBottom - viewHeight + scrollTop + marginBottom, scrollUp = scrollTop + menuTop - marginTop;
  switch (placement) {
   case "auto":
   case "bottom":
    ...
    if (scrollSpaceBelow - marginTop - marginBottom >= menuHeight && !isFixedPosition) return shouldScroll && utils.animatedScrollTo(scrollParent, scrollDown, 160), 
    {
      placement: "bottom",
      maxHeight: maxHeight
    };
    if (!isFixedPosition && scrollSpaceBelowMenu >= minHeight && viewHeight + scrollSpaceBelow >= scrollSpaceBelowMenu || isFixedPosition && viewSpaceBelow >= minHeight) return shouldScroll && utils.animatedScrollTo(scrollParent, scrollDown, 160), 
    {
      placement: "bottom",
      maxHeight: isFixedPosition ? viewSpaceBelow - marginBottom : Math.min(scrollSpaceBelowMenu - marginBottom, maxHeight)
    };
